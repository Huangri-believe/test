...
def mv_data_idx(src_dir, tgt_dir, idx_delta):
    ...
    for file_name in sorted(os.listdir(src_dir)):
        ...
        if file.suffix == ".parquet":
            ...
            df['episode_index'] = new_index  # 修改 episode_index
            df.to_parquet(tgt_dir / new_name, index=False)  # 保存到目标目录
        else:
            shutil.copy2(file, tgt_dir / new_name)
...
def mv_data_dir_idx(src_dir, tgt_dir, idx):
    ...
    for dir_name in sorted(os.listdir(src_dir)):
        ...
        shutil.copytree(file, tgt_dir / new_name, dirs_exist_ok=True)
...
def mv_data(src_dir, dst_dir):
    ...
    for file_name in sorted(os.listdir(src_dir)):
        ...
        if file.is_file():  # 只处理文件，跳过子目录
            shutil.copy2(str(file), dst_dir / file.name)
